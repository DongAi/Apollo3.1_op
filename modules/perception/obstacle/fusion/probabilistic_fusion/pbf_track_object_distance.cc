...
float PbfTrackObjectDistance::ComputeDistanceAngleMatchProb(
  ...
  if (fcenter(0) > epislon && std::abs(fcenter(1)) > epislon) {
    float x_ratio = std::abs(fcenter(0) - scenter(0)) * (1.0 / fcenter(0));
    assert(x_ratio >= 0);
    float y_ratio = std::abs(fcenter(1) - scenter(1)) * (1.0 / std::abs(fcenter(1)));
  ...
  if (svelocity > 0.0 && fvelocity > 0.0) {
    float velocity_m = 1.0f / (svelocity * fvelocity);
    float cos_distance =
        sobj->velocity.dot(fobj->velocity) * velocity_m;
